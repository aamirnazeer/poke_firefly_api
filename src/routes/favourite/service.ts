...
import {
  setFavouritesController,
  getFavouritesController,
  checkFavouritesController,
  getFavouritesCount,
...
export const setFavouritesService = async (name: string, uuid: string) => {
  ...
  if (count)
    throw new CustomError(StatusCodes.CONFLICT, "already set as favourite");
...
export const getFavouritesService = async (
  ...
  const results = await getFavouritesController(uuid, offset, limit);
...
export const deleteFavouriteService = async (name: string, uuid: string) => {
  const count = await checkFavouritesController(name, uuid);
  if (!count)
    throw new CustomError(StatusCodes.CONFLICT, "favourite not available");
