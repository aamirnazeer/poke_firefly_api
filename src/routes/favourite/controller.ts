...
export const setFavouritesController = async (name: string, uuid: string) => {
  return db.insert(favourites).values({ pokemon_name: name, user_id: uuid });
};

export const checkFavouritesController = async (name: string, uuid: string) => {
  return db.$count(
    favourites,
    and(eq(favourites.user_id, uuid), eq(favourites.pokemon_name, name))
  );
};
...
export const getFavouritesController = async (
  ...
  return db
    .select({ name: favourites.pokemon_name })
    .from(favourites)
...
export const deleteFavouriteController = async (name: string, uuid: string) => {
  await db
    .delete(favourites)
    .where(
      and(eq(favourites.pokemon_name, name), eq(favourites.user_id, uuid))
    );
};
